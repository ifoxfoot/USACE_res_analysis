...
res_clean <- res_raw %>% 
  ...
  sf::st_make_valid() %>% #make sure all shapes are "valid" geometry
  filter(dry != "Yes") #remove dry lakes

#temporary
res_clean <- res_clean %>% 
  mutate(area = st_area(res_clean))

#clarify duplicate names
res_clean_name <- res_clean %>% 
  mutate(name = ifelse(
    district == "Tulsa District" & name == "Sardis Lake",
    "Sardis Lake Tulsa", name)) %>% 
  mutate(name = ifelse(
    district == "Vicksburg District" & name == "Sardis Lake",
    "Sardis Lake Vicksburg", name))
